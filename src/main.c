...
int main() {
    ...
    int status = 0; //pour stocker le statut de la dérniére commande executée 
    static char previous_dir[CWDSIZE] = "";//tampon pour le dérnier répertoir qu'on a visiter 


    while (1) {
        prompt(status); ///affiche le prompt personalisé et le statut de la dérniére commande
